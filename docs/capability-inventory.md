...
| Boot project info | `planned` | `sts/spring-boot/bootProjectInfo`. The server command is advertised and forwarded unchanged, but the product has no user-facing Zed command, task, or DAP consumer yet; server availability alone does not implement the outcome. |
| Executable Boot projects discovery | `planned` | `sts/spring-boot/executableBootProjects`. Its required server-to-client `sts/project/gav` callback is now contract-tested through the coordinator and official Java transport as `sts.project.gav`. A user-facing Zed consumer and a driven result on a named tuple remain required. |
