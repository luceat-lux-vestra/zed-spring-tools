...
| Inlay hints (including cron) | `planned` | Server advertises `inlayHintProvider`; setting `boot-java.cron.inlay-hints`. **Observation, not yet a verdict:** in a valid driven run (2026-07-18, Spring up, `inlay_hints.enabled` on) Zed issued zero `textDocument/inlayHint` on the Java buffers while issuing `textDocument/codeLens` in the same window. But this is **not** established as `blocked-zed-api`: (a) it is not a capability gate — Zed's source honors `inlayHintProvider` from both the static initialize result (Spring) and dynamic `client/registerCapability` (jdtls), and `InlayHints::check_capabilities` passes for `Left(true)` (`lsp_command.rs`); the editor resolves inlay-hint enablement per language via `language_settings_at().inlay_hints` (`editor/src/inlays/inlay_hints.rs`); (b) the follow-up "confirmation" runs were **invalid** — they launched a bare file without the worktree folder, so the coordinator failed (`worktree is not an absolute directory`) and the Spring server never started, so their zero-request counts prove nothing; and the Java-buffer observation is multi-server (jdtls+Spring), never cleanly isolated to a single Spring-only Properties buffer with Spring running. A clean re-test (worktree folder open, Java file to bring jdtls up, then observe the Properties buffer) is required before any verdict, and the `initialization_options`/per-language `inlay_hints` levers must be tried. Evidence: `tmp/lsp-verify-20260718/`. |
