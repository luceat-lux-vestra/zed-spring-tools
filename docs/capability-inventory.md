...
Second, the run found two upstream defects
(`getResolvedProjectClasspath` throwing on a null classpath-entry version, and
`getLatestReleaseInformation` returning `null`), and the same run carries the
evidence that the first is not ours: the classpath is demonstrably delivered,
because `getSpringBootVersion` and `getJavaVersion` are read from it and both
resolved. Neither defect blocks the row; both are named in it.
