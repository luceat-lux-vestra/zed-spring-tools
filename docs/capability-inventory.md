...
readable. Three limits are recorded rather than smoothed over: the gate drove
the server directly over LSP instead of through Zed's UI, the target was
localhost rather than a physically remote host, and only the `http` branch of
