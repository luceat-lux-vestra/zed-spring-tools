...
| Run / debug a Boot application | `planned` | S016 verified official Java 6.8.23's matching Maven main runnable through ordinary-profile Zed on macOS arm64/JDK 25: it launched the Boot fixture, served `GET /greeting`, and stopped through the task terminal. The shipped product still declares 6.8.21, and debug remains untested, so the capability state does not change. After the compatibility-table change, reuse the official runnable for matching Run actions. Reviewable Spring-specific `.zed/tasks.json` and explicit official-Java Run (`noDebug`) or Debug entries are companions only where profiles, arguments, project choice, or debugging require them. No route may overwrite unknown configuration or imply programmatic debug start. |
| Maven goal / Gradle build | `planned` | Build execution remains official Java/Zed task ownership under D003. S016 verified Maven main execution through `compile exec:java` and verified Gradle project coordination, but did not run a Gradle/vanilla task or Java test task. After the compatibility-table change, prefer official Java 6.8.23's wrapper-aware tasks only where matching runtime evidence exists; generate or merge reviewable `.zed/tasks.json` for arbitrary goals/builds or Spring-specific commands. Manual tasks remain fallback. Spring LS's direct `Runtime.exec` commands are not selected because they do not provide Zed task/terminal ownership. |
...
| Offline behaviour | `planned` | Preserve the checked artifact cache, compatibility diagnostics, and fail-closed provider behavior. S016 verified one warm cached startup with outbound network denied for official Java 6.8.23 on macOS arm64/JDK 25. The product still does not declare 6.8.23, and rollback, first-install offline behavior, and other platform evidence remain planned; see `LIMITATIONS.md`. |
