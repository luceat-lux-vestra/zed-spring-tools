...
- Derived from: Spring Tools `5.2.0.RELEASE` / `vscode-spring-boot` `2.2.0`.
  **The product pin moved to `5.3.0.RELEASE` / `2.3.0` on 2026-08-01, and the
  ...
  changed. The refresh found one regression — remote connect no longer attaches on
  declaration — but it costs a step rather than an outcome, so the row keeps its
  state and carries the change in its notes.
...
- Build-system coverage: [Gradle axis resolution](gradle-axis-resolution.md)
...
navigation. **That Zed limitation is fixed upstream and the fix is not in any
tested release**: [zed#61784](https://github.com/zed-industries/zed/pull/61784)
landed on `main` on 2026-07-28, after 1.12.1, and carries a test asserting the
fragment is honoured. Re-test on the next Zed release and drop this sentence
rather than filing anything. `structure/groups` visibility selection is not part
of this first all-default-groups prototype.
...
but, past the pipe buffer, unfinishable. Filed upstream on 2026-07-29 as
[spring-tools#1950](https://github.com/spring-projects/spring-tools/issues/1950),
with the mechanism read from the jar — `Runtime.exec` gives the child pipes and
nothing ever reads them, and `executeGradle` has the same shape. That filing
changes nothing here. The coordinator now answers both
...
One cost is recorded rather than discovered later: connecting Zed's MCP client
to the resulting endpoint is user configuration, not something the extension
performs. That belongs in release-facing documentation and is not a blocker.

Version 46 also claimed this slice would need a settings layer the extension
does not have, on the grounds that every `boot-java.*` key so far is forwarded
*to* the server through `spring_workspace_configuration`. **That was wrong**, and
building it proved so: `boot-java.live-information.automatic-connection.on` was
already read from `LspSettings` at launch and passed to the coordinator as
`--automatic-live-connection`. The MCP settings took that same established
route, which made the slice smaller than the entry predicted rather than larger.

Inventory version 47 builds it, the same day, and moves the row to
`implemented`. The slice is what version 46 described and nothing more: both
settings are read at launch, resolved into a single `--mcp-server-port`
coordinator argument (`off`, or a port), and applied in `springArguments`. With
the setting absent the argument vector is byte-identical to every release before
it, and a contract test asserts exactly that rather than leaving it to
inspection. Two departures from upstream's own expression are deliberate and
tested: its `port <= 655536` bound is past the last valid TCP port, so a user
who wrote `70000` would fail the whole server start instead of getting a
disabled MCP server, and a malformed value disables the server rather than
substituting the default port. `verified` still requires index-backed tool
payloads against a resolved project on a named tuple.

Inventory version 48 supplies exactly that and promotes the row to `verified`.
All 18 tools were driven against a resolved `practices-app`, and the payloads are
real: beans with source ranges and injection points, the `/hello` `GET` mapping,
4 838 B of diagnostics, and live `api.spring.io` data. A negative control on an
unknown project name was refused, so the result does not rest on "something came
back".

Two things this run did that are worth keeping as method. First, a suspected
cleanup regression was **measured rather than argued**: Tomcat's acceptor thread
is non-daemon and one post-quit check saw the Spring process at +6 s, so an A/B
on the one setting was run — 3.70 s off versus 3.66 s on, no orphan. The
suspicion was reasonable and wrong, which is the outcome worth recording.
Second, the run found one upstream defect
(`getResolvedProjectClasspath` throwing on a null classpath-entry version), and
the same run carries the evidence that it is not ours: the classpath is
demonstrably delivered, because `getSpringBootVersion` and `getJavaVersion` are
read from it and both resolved. It does not block the row and is named in it.
The run also claimed a second defect, `getLatestReleaseInformation` returning
`null`, which version 53 withdraws as a harness error — the call passed the IDE
workspace name where the tool documents the Spring portfolio id.

Inventory version 49 changes no state. It closes the **build-system axis**,
which was orthogonal to every state in this table and was, until 2026-07-29,
entirely unobserved: every driven gate in versions 1-48 used a Maven fixture, so
"verified" silently meant "verified on Maven". The
[Gradle axis resolution](gradle-axis-resolution.md) carries the survey, the
method, and the full evidence; this note records only what it changes here.

The survey's own result is worth keeping, because it is why this did not cost 59
gates. A capability is build-system-sensitive only if some code branches on the
build tool, reads the build file, or writes a build command; everything else
reaches Spring through the jdtls classpath, where Maven and Gradle have already
been normalised into one `IJavaProject`. That reduced 59 rows to nine that
needed their own evidence and one that is Maven-only in upstream's code. The
class claim was then driven rather than asserted, and nothing refuted it.

Two of the plan's own characterisations of this axis were wrong and are
corrected by the rows below. **Modulith metadata generation is not Maven-only**:
`ModulithService` builds a classpath string from `IClasspath` entries and spawns
`ApplicationModulesExporter` against it without reading a build file at all, so
the precondition is compiled classes, and `./gradlew classes` satisfies it.
**The Boot upgrade does not "reject Gradle outright"** in the sense of failing at
the user: `UpdateBootVersion.canProvideQuickfix` is `"maven".equals(build type)`
and is consulted *before* the upgrade action is attached, while the release-notes
action is added outside that branch — so a Gradle user gets the diagnostic and
one action, never an error. That also makes this extension's upgrade
failure-visibility path unreachable on Gradle, which is correct rather than a
hole.

What the gate could not close is stated where it belongs rather than smoothed
over: the Windows wrapper forms `mvnw.cmd` and `gradlew.bat` need a Windows
host, so that sub-axis stays blocked on the same hardware M5 slices 2-4 are, and
multi-project Gradle selection remains contract-tested only.

Inventory version 50 changes no state either. It corrects two factual claims in
the **completion prefix elision** row, both of which had been asserted from the
outside and neither of which survived reading the pinned server jar and this
extension's own settings path. The row said the setting is not user-toggleable
through Zed settings; `merge_configuration` is a generic recursive deep merge, so
it always was. The row said turning it on risks duplicate-prefix insertion like
`server.server.port`; `DocumentEdits.dropPrefix` guards on the document text
actually starting with the prefix and then moves the deletion start and the
insert text together, so that failure mode is not reachable. The correction
matters beyond one row because both errors share a shape this inventory is meant
to prevent: a risk was inferred from what a setting sounded like it would do,
recorded as a reason not to act, and then cited as settled. The replacement text
names the mechanics actually read — including that elision never runs when
navigation proposals are returned — and states the one question that is still
open, which is about Zed's completion matching rather than about text
corruption. The state stays `not-pursued` on the unchanged ground that VS Code
defaults the setting off, so parity is already met.

Inventory version 51 **does** change state, and it changes one this inventory had
declared settled: **embedded language syntax highlighting moves from
`blocked-zed-api` to `verified`**, driven on 2026-07-29. The blocker never
existed. S017 concluded that Zed issues no `textDocument/semanticTokens` request
for a Java buffer under any registration shape, and rested that on what looked
like an unanswerable control — jdtls, the primary server, declares the provider
statically and drew no request either. Zed's `semantic_tokens` **language setting
defaults to `"off"`**, and `"off"` is documented as "do not request semantic
tokens from language servers". The control was not evidence against a Java
token path; it was the global switch being off for every server at once, which
is exactly the shape the same switch produced for `document_symbols` and is
already recorded as a correction in [LIMITATIONS](../LIMITATIONS.md). Upstream
had shipped the feature before the spike ran (`zed#7450`, closed 2026-02-06) and
had fixed dynamic registration handling before this month (`zed#60015`, merged
2026-06-28), so nothing about Zed changed to make this work.

The correction is worth more than one row. Three rows here cite S017 as
established fact, and two of the surviving `blocked-zed-api` rows were reasoned
about by analogy to it — *Spring-specific document highlights* in particular was
recorded as "the same class of missing surface". It is not the same class, and
the difference is now read from Zed's source rather than inferred: semantic
tokens fan out to **every** capable server
(`fetch_semantic_tokens_for_buffer` iterates `local_lsp_servers_for_buffer` and
returns a per-server map), while document highlights take
`LanguageServerToQuery::FirstCapable` and stop at jdtls. Those two rows stay
blocked on their own separately verified evidence, not on this one's.

Inventory version 52 changes no state, and finishes what version 50 started. The
**completion prefix elision** gate that version 50 recorded as designed-but-unrun
was run on 2026-07-29, and it did three things. It confirmed both of version 50's
corrections by observation rather than by reading — the setting reaches the
server through the generic deep merge, and the elided edit is self-consistent.
It measured when the feature fires at all: on a plain `server.` prefix it does
nothing, because fuzzy matches like `spring.rsocket.server.port` collapse the
shared prefix, but one character past a dot it elides every item. And it found
the thing no amount of reading upstream would have produced — across six such
queries carrying 159 elided items, with the setting on Zed shows **no completion
menu at all**, where the same six carets with it off each produce a menu and
accept the full key. The setting is not cosmetic in Zed; it removes working
completions. Reading Zed's source then named the cause exactly — `filterText` is
honoured only when it is a substring of the displayed label, and elision makes it
a superstring — which turns this row from "we observed something bad" into a
condition someone can check for in a future Zed release. That is now the
row's reason, replacing "parity is already met", which was true but weak.

Two process points survive this. First, the gate was worth running even though
the row's state did not change, because a state can be right for a reason that
is wrong, and the reason is what the next person reads. Second, the run's own
first arm produced a clean-looking trace with no menu ever displayed, because
the gesture key never reached Zed — and it was caught only because the *control*
arm failed the same way. A completion request in the log is evidence the server
was asked, never evidence the user would have seen anything, so any conclusion
drawn from an absent popup needs a positive control before it is written down.

Inventory version 53 changes no state either. It records where four defects
this project found now live upstream, after each was re-verified on 2026-07-29
against the pinned jars or Zed's current `main` —
[spring-tools#1949](https://github.com/spring-projects/spring-tools/issues/1949)
and [#1950](https://github.com/spring-projects/spring-tools/issues/1950),
[zed#61863](https://github.com/zed-industries/zed/issues/61863) and
[#61865](https://github.com/zed-industries/zed/issues/61865) — and it drops one
candidate that no longer needs filing, because Zed fixed the Markdown `#L…`
fragment upstream after 1.12.1.

The result worth generalising is how the MCP classpath defect was attributed.
The open question was whether the null came from *our* bridge, since this
project supplies the classpath that VS Code gets from `vscode-java`, and the
planned answer was an expensive cross-check in VS Code. Reading the jar was
cheaper and stronger: the field is `transient`, so it is never on the wire, and
the value is derived inside the server from the jar's file name. **A defect
downstream of a transient or derived field cannot be the client's**, which
settles attribution without running the other client at all. Ask what supplies
a value before building a comparison that assumes it travels.

None of the four affects this extension's users, and that is worth stating
plainly next to the filings: the classpath tool is exposed to the Agent and
never called by our code, the build commands are intercepted before they reach
the handler that deadlocks, and the two Zed rows were already
`blocked-zed-api`. Filing them is contribution, not maintenance.

A fifth candidate was withdrawn instead of filed, and that is the more useful
half of this version. `getLatestReleaseInformation` had been recorded as a
second MCP defect because it returned `null` where a sibling tool answered for
"the same project". Re-driving it on 2026-07-30 showed the two tools do not take
the same argument at all: this one wants the Spring portfolio id (`spring-boot`,
`spring-framework`), which its own `@ToolParam` says in as many words, and the
sweep had passed the IDE workspace name. Called correctly it answers with the
current release and both support end dates. The tell was in the number the row
already carried and nobody read as a signal: 8 ms with no network is not a slow
lookup that missed, it is no lookup at all. **Before reporting a tool as broken,
re-read the argument contract it publishes** — and treat a suspiciously cheap
failure as evidence the call never happened.

The standing rule this breaks is the one the version 50 note had just written
down: a limitation was inferred from an observation without checking the client
setting that governs it. The habit that catches it is cheap — before recording a
client-side blocker, read the client's default settings and its current source,
not only the trace.

Inventory version 54 changes no state, and it is the first version produced by a
**pinned-release refresh** rather than by work on a capability. The product moved
from `5.2.0.RELEASE` to `5.3.0.RELEASE` on 2026-08-01 through the
[refresh gate](pinned-release-refresh-gate.md), which found one regression in
one row and nothing else: the declared surface is identical key for key, and the audit's two highest-risk
implementation changes — a rewritten `Version` class and a relocated live-process
attach — reproduce the previous release's observable behaviour exactly. No row
moved, so this version's content is the *evidence-release* rule now in the header:
a row that was not re-run still says `verified`, and what that means is that its
evidence belongs to `5.2.0.RELEASE` and is untested against the new pin, which is
not the same statement as failing.

Two things generalise beyond this refresh. First, **an upstream defect can be
fixed by a refresh, and its milestone label will not tell you** — spring-tools#1949
is fixed in `5.3.0.RELEASE` while labelled `5.4.0.RELEASE`, and had the label been
trusted the README would have kept advertising a defect the shipped product does
not have. Checking reported blockers against the release rather than the label is
now an explicit stage of the gate. Second, **"no regression" is not "no work"**:
the identity moved, so every claim anchored to the old identity is either
re-established or silently stale, and nothing fails to make that visible.

Inventory version 55 closes the refresh's residual Tier B and changes no state,
though it came closer than any version so far. Four of the five groups the first
pass left unmeasured — the Java reconcilers, Spring AI, the live-data family, and
the workspace-symbol/index-cache/Structure surfaces — reproduce `5.2.0.RELEASE`
byte for byte on `5.3.0.RELEASE`, so R021's two structural findings (ten new
reconciler version literals, fifteen changed index-cache classes) turn out to
change no observable outcome at all. The fifth, remote connect, lost its
automatic attach on declaration and kept everything else; the row stays
`verified` because the target is still offered in a verified action and one click
connects it.

Three method lessons, none of them about Spring. **A/B beats a recorded
baseline.** The reconciler pass first ran against the fixture its own generator
produces and came up two problem types short — and the two absences were present
in both arms, which is what stopped them being read as a regression. The cause
was that `make-fixture.sh` has drifted from the fixture snapshot beside it in the
same evidence directory, most importantly writing a `SecurityManager` field where
the snapshot throws `AccessControlException`, and the type *is* the trigger. A
comparison against the recorded 28 types would have called that a 5.3.0
regression. **Read argument shapes from the artifact.** The refresh gate could
not measure metrics, loggers or live data because its probe guessed the command
arguments and every call timed out; the shapes are in the server jar's own string
pool and in the VS Code client's call sites, and with them every route answered.
**And a negative result can be the harness.** Automatic connection does not fire
under a bare LSP probe on either release, because without jdtls there is no
resolved project to bind a process to — it took the real product to show the row
working.
...
| Completion prefix elision | `not-pursued` | Setting `boot-java.properties.completions.elide-prefix` (VS Code default `false`). It does not change *which* keys are proposed, only how each item is labelled and edited: with it on, candidates under an already-typed dotted prefix (`server.` → `server.port`, `server.address`) are labelled by their tail (`port`, `address`) so the shared prefix is not repeated on every row. **Two earlier claims in this row were wrong, and both were corrected on 2026-07-29 by reading the pinned 5.2.0 server jar and this extension's own settings path; the state is unchanged, only its reasons.** *Correction 1 — the value is reachable from Zed settings today, with no product code.* `merge_configuration` ([src/lib.rs](../src/lib.rs)) is a generic recursive deep merge of the user's `lsp."spring-tools".settings` over our defaults, not a fixed key list, so `boot-java.properties.completions.elide-prefix` written in Zed settings is forwarded verbatim to the server like any other `boot-java.*` key. *Correction 2 — the "duplicate-prefix insertion like `server.server.port`" risk was misread.* `DocumentEdits.dropPrefix` fires only when the edit list is exactly `[Deletion, Insertion]`, the insertion offset lies inside the deletion range, **and** the document text in that range actually starts with the prefix; it then advances `Deletion.start` by the prefix length *and* strips the same prefix from the insert text, so the range and the text stay consistent, and when the guard fails it silently does nothing. The mechanics that do matter are different from what this row assumed. `getPropertyCompletions` returns `getNavigationProposals()` whenever that is non-empty and only otherwise calls `getFuzzyCompletions()`, so **elision never runs on the navigation path**; inside the fuzzy path, `elideCommonPrefix` takes the common prefix of the typed query *plus every proposal label* and elides only up to its last `.`, so a query whose matches do not all share a dotted prefix is left untouched. `dropLabelPrefix` wraps each proposal in a `TransformedCompletion` whose `getFilterText()` delegates to the **original**, so the wire item carries an elided `label` with the full key still in `filterText`. That is the real open question, and it is the opposite of the one this row used to state: not corrupted text, but whether Zed's own fuzzy matcher — which outranks the server's `sortText` — matches against `filterText` or the label, because matching on the label would drop items from the menu once the typed query no longer appears in it. **The driven gate ran on 2026-07-29, and it changes the reason this row is `not-pursued` from "no gap" to "enabling it makes Zed worse."** Tuple: macOS 26.5.x arm64, Zed 1.12.1, official Java 6.8.21 (jdtls 1.60.0), Spring Tools 5.2.0, Temurin JDK 25.0.3, against the Boot 3.5.5 `spring-boot-basic` Maven fixture, A/B on one run dir with the setting as the only difference. *Correction 1 is now observed, not just read*: `"elide-prefix":true` appears in the `workspace/configuration` answer, so the generic deep merge does deliver an arbitrary `boot-java.*` key with no product code. *Correction 2 is confirmed*: where elision fires the edit is self-consistent — `label` `jpa.hibernate.ddl-auto`, `filterText` the full `spring.jpa.hibernate.ddl-auto`, `newText` the elided tail, and the replace range moving from `0–13` to `7–13` so the typed `spring.` is preserved rather than re-inserted. Nothing resembling `server.server.port` occurs. **Whether elision fires depends on how specific the typed key is**, which the gate measured rather than assumed. On the shallow `server.` (261 items) and `server.p` (124 items) it did not fire at all, because the fuzzy match set includes 68 and 40 keys such as `spring.rsocket.server.port`, which collapses the common prefix of query-plus-labels to `s` — no dot, no elision. Once the query is specific enough that the match set stays inside one dotted namespace it fires reliably: six queries typed one character past a dot (`logging.`+`l`, `spring.mvc.`+`s`, `server.tomcat.`+`a`, `spring.datasource.`+`h`, `spring.jpa.hibernate.`+`n`, `spring.jpa.`+`h`) elided **every** item, 159 of 159. **The blocking result is what Zed then does with those items: nothing.** Those same six queries were run in both states, same carets, same fixture, same typed triggers, the setting the only difference. With it off, all six produced a menu and accepting the top item inserted the full key — `logging.level.`, `spring.mvc.servlet.load-on-startup=`, `server.tomcat.accept-count=`, `spring.datasource.hikari.allow-pool-suspension=`, `spring.jpa.hibernate.naming.implicit-strategy=`, `spring.jpa.hibernate.ddl-auto=`. With it on, **none of the six produced a menu at all** and `Enter` fell through to a newline every time, while the server had returned all 159 items. Five of the six returned an identical item count in both states (18, 49, 47, 3, 6), so the match sets are the same and the elision transform is the only variable. So the setting does not degrade formatting, it silently removes working completions. **The Zed-side cause is identified**, by reading Zed's own source at the tested tag `v1.12.1` (the code is byte-identical on `main` at `007ffc790e`). It is not the replace range: it is that Zed accepts an LSP `filterText` **only when that string occurs inside the text it displays**, and otherwise silently falls back to the label — `item.filter_text.as_deref().and_then(|filter| text.find(filter).map(|ix| ix..ix + filter.len())).unwrap_or(0..label_length)` in `CodeLabel::fallback_for_completion` (`crates/language/src/language.rs`), with the menu then matching on `CodeLabel::filter_text()` through `StringMatchCandidate::new(id, completion.label.filter_text())` (`crates/editor/src/code_context_menus.rs`). Elision inverts that containment: it makes `filterText` a *superstring* of the label (`spring.jpa.hibernate.ddl-auto` against a label of `jpa.hibernate.ddl-auto`), so `find` returns `None`, the elided label becomes the match text, and the user's typed `spring.jpa.h` cannot match it. Every item is filtered out, which is why the menu never appears. LSP defines `filterText` as the string to filter against and falls back to the label only when it is *absent*; the substring precondition is Zed's own. This also rules out a workaround here: the coordinator can rewrite items, but no `filterText` it could send is both a substring of an elided label and a match for the unelided text the user typed. **The reading is independently corroborated and the defect is already tracked upstream**: [zed-industries/zed#61646](https://github.com/zed-industries/zed/issues/61646), opened 2026-07-25 by an unrelated reporter, names the same three code sites and the same spec argument, and a contributor has taken it. Nothing here is Spring-specific, so no issue was filed from this project; watch that one instead, and treat its fix as the trigger to re-run this gate. Still `not-pursued`, and deliberately not `blocked-zed-api`: that label is for a capability this project wants and cannot have, whereas VS Code defaults this one off, parity is met by not shipping it, and the gate found that shipping it would regress the outcome. A future reversal now needs Zed to change, not this project: the setting becomes shippable the day `filterText` is honoured without the substring precondition, and reopening this row should start by re-reading that one expression. **Limits**: one tuple, one Zed version, one Maven fixture, and `.properties` only — the YAML completion path was not exercised, and neither was a project whose metadata comes from the user's own `@ConfigurationProperties` rather than from Boot's. The six queries all elide a *leading* namespace; no case was found where elision cuts at a deeper dot than the query's own last one. Evidence: `tmp/elide-verify-20260729/evidence/` (both traces, both menu screenshots, the arm-B settings file). Harness note for whoever re-runs it: the run's first arm produced no menu in *either* setting state, because `ctrl-space` was bound to input-source switching in the tested host's own keyboard settings and never reached Zed as `editor::ShowCompletions`. That binding has since been removed, so `ctrl-space` is not a standing limitation — but the diagnostic lesson is, because caret placement alone makes Zed request completions: a `textDocument/completion` request and a full response appear in the trace whether or not a menu was ever shown, so the log looks healthy while the user sees nothing. Trigger by typing a character, and confirm with a screenshot plus a `Return` that either applies an item or falls through to a newline. |
...
| Spring-specific document highlights | `blocked-zed-api` | Spring's WebFlux-route and embedded-query document highlights cannot reach stock Zed's Java buffer: S018 observed steady-state `textDocument/documentHighlight` requests going only to the primary jdtls server, never to the Spring coordinator. Do not add coordinator merge code. **Re-checked against Zed's source on 2026-07-29, when the neighbouring semantic-token row was overturned, and this one holds**: `Project::document_highlights` passes `LanguageServerToQuery::FirstCapable`, so exactly one server is asked — unlike semantic tokens, which fan out to every capable server, and unlike references, which Zed unions. This row is therefore not "the same class" as that one, as S018 originally put it; it is a single-server request path in the client, and no client setting suppresses it. Reopen if Zed aggregates document highlights across language servers in a future release; ordinary jdtls highlights remain available. Filed as [zed#61865](https://github.com/zed-industries/zed/issues/61865), framed as the inconsistency it is — `definitions` and `references` fan out through `request_multiple_lsp_locally` and dedup, this path does not — with an explicit invitation to close it as intended behaviour and document it instead, since from here "the server was never asked" and "the server has no highlights" are indistinguishable. |
...
| Embedded Spring Tools MCP server | `verified` | Verified 2026-07-29 on macOS 26.5.2 arm64, Zed 1.12.1, official Java 6.8.21 (jdtls 1.60.0), Spring Tools 5.2.0, Temurin JDK 25.0.3, against the Boot 3.5.5 `practices-app` Maven fixture with the shipped opt-in setting written into the profile as a user would write it. **The setting reaches the JVM**: `-Dserver.port=50627` present, `-Dspring.main.web-application-type=NONE` absent, `window/showMessage` announcing the *configured* port rather than a random one, a `127.0.0.1:50627` listening socket, `project created event: practices-app`, and LSP still publishing diagnostics afterwards. **All 18 tools were exercised against the resolved project**, and 17 answered with real payloads: `getProjectList` → `practices-app` with `isSpringBootProject` and JRE 21.0.11; `getSpringBootVersion` → 3.5.5; `getRequestMappings` → the `/hello` `GET` mapping under controller `/greetings`; `getBeanDetails` → 26 607 B of indexed beans with source ranges; `getBeanUsageInfo` → a definition **plus its injection points**; `findComponentsByStereotype` → `GreetingController`; `getProjectDiagnostics` → 4 838 B of real diagnostics with URIs and positions; and the five `api.spring.io` tools returning live release, generation and calendar data. The latency split is corroborating evidence — index tools answer in 5-91 ms, the live-calendar tools take 1.2-1.7 s. A negative control (`projectName: "no-such-project-xyz"`) was refused with `project with name ... not found`, so "it returned something" is not what this rests on. `prompts/list` returns the one bundled prompt. **Cleanup was measured, not assumed**: a suspected regression (Tomcat's acceptor is non-daemon, and one post-quit check saw the process at +6 s) did not survive an A/B — Spring exits in 3.70 s with the setting off and 3.66 s with it on, no orphan and no held port. **One upstream defect found.** `getResolvedProjectClasspath` returns `isError` with `Cannot invoke "…Version.toString()" because … Classpath$CPE.getVersion() is null` — the tool maps entries without a null check, and the same run shows the classpath itself is fine, because `getSpringBootVersion` and `getJavaVersion` are read from it and both resolved; the bridge forwards upstream's own `ReusableClasspathListenerHandler` payload untransformed. **Root cause read from the pinned jars on 2026-07-29, and it removes the "unconfirmed against VS Code" caveat this row used to carry**: `Classpath$CPE.version` is a `transient` field, so it never travels with the classpath payload and no client can supply or influence it; `getVersion()` derives it *inside the language server* from the entry's **file name** through `Classpath.getDependencyVersion`, whose regex `^.+-(0|[1-9]\d*)\.(0|[1-9]\d*)\.(0|[1-9]\d*)…\.jar$` returns `null` on any name that is not strict three-part SemVer; and `ProjectInformation.getResolvedProjectClasspath` then calls `.toString()` on it unguarded for every `kind == "binary" && !isSystem()` entry. The defect is therefore client-independent by construction. The trigger is ordinary: of the 63 jars on this fixture's compile classpath exactly one fails that pattern — `snakeyaml-2.4.jar`, a two-segment version reached transitively through `spring-boot-starter` — which is enough to fail the whole call on essentially any Boot application. Filed as [spring-tools#1949](https://github.com/spring-projects/spring-tools/issues/1949). **That defect is fixed in `5.3.0.RELEASE`, the release this product pinned on 2026-08-01** — driven that day with the same `snakeyaml-2.4.jar` trigger on the classpath, the call answers with no error and the entry's version arrives as a JSON `null`. Upstream milestoned the issue `5.4.0.RELEASE`, so the label alone says the opposite; the mechanism is a return-type change in `Classpath$CPE.getVersion()` read from the pinned jars. This row's own evidence remains `5.2.0.RELEASE`, per the evidence-release rule in the header. **The second defect this row used to claim was withdrawn on 2026-07-30: it was a harness error, not upstream's.** `getLatestReleaseInformation` was recorded as returning `null` in 8 ms where its sibling `getLatestBootVersionsFromMavenRepo` answered for the same project name. The sibling comparison is what made it look like a defect, and it is exactly what does not hold: the two tools take different arguments. This one's `@ToolParam` reads "Spring project technical id, e.g. \"spring-boot\", \"spring-framework\", \"spring-data-jpa\" (not the IDE workspace project name)", and the sweep passed the IDE workspace name `practices-app`. Re-driven 2026-07-30 against the same pinned jar through the same HTTP harness: `spring-boot` returns `{"version":"4.1.0","endOfOssSupport":"2027-07-31","endOfCommercialSupport":"2028-07-31"}` in 1 866 ms, `spring-framework` returns `7.0.8` in 713 ms, and `practices-app` returns `null` in 6 ms — the 8 ms with no network was never a lookup miss, it was no lookup at all. So **one** upstream defect stands in this row, not two, and the only residual observation is a small inconsistency that helped the mistake survive: an unknown id is answered with a silent `null` here, while `getSpringBootVersion` refuses an unknown project explicitly. Not worth filing, and recorded so the withdrawal is not re-derived as a discovery. Evidence: `tmp/scratch/s3check/`. **Gradle observed 2026-07-29**: through the same shipped opt-in setting on `tests/fixtures/spring-boot-gradle`, `getProjectList` returned the Gradle project with `isSpringBootProject` and JRE 21.0.11, `getSpringBootVersion` → 3.5.0 (the version the Gradle build pins, so it is read from that build), `getRequestMappings` → the `/greeting` `GET` mapping, `getBeanDetails` → 27 895 B with source ranges, and `getProjectDiagnostics` → 11 932 B. `getResolvedProjectClasspath` fails on Gradle with the **identical** null `Classpath$CPE.getVersion()` dereference, which is useful corroboration that the defect is upstream's missing null check rather than something about Maven-shaped input. **Limits**: one tuple, one project per worktree; multi-project worktrees unobserved; no MCP client was connected, because wiring Zed's own remains user configuration; the prompt was listed but not executed. Evidence: `tmp/mcp-verify-20260729/evidence/`. Spring Tools 5.2.0 contains an experimental streamable-HTTP MCP server exposing 18 tools and one prompt — project list, Boot/Java version, resolved classpath, bean details and usage, beans by type, request mappings, stereotypes and components, Spring Tools diagnostics, plus five `api.spring.io` release/generation tools — and Zed supports remote MCP tools and prompts. **Decided 2026-07-29 (inventory version 46): build it, honouring upstream's two settings, defaulting off. Built the same day (inventory version 47), so the row is `implemented`: `boot-java.ai.mcp-server-enabled` and `boot-java.ai.mcp-server-port` are read from `LspSettings` at launch, resolved into one `--mcp-server-port` coordinator argument, and applied in `springArguments` as `-Dserver.port=<port>` in place of `-Dspring.main.web-application-type=NONE`. With the setting absent or false the launch vector is byte-identical to every release before it, which a contract test pins directly. Two deliberate departures from upstream's expression are tested: its `port <= 655536` bound is past the last TCP port, so the bound here is 65535, and any malformed value disables the server instead of falling back to the default port — opening a socket the user did not write is the wrong failure direction for a network-facing option. `verified` additionally requires index-backed tool payloads against a resolved project on a named tuple, which nothing has observed yet.** [S019](spikes/019-embedded-mcp-server.md) observed the capability working: dropping `-Dspring.main.web-application-type=NONE` from `springArguments` (`coordinator/src/main.mjs`) is by itself sufficient, the unmodified jar then bound `127.0.0.1:<port>` and returned all 18 tools over `POST /mcp`, and the same JVM served stdio LSP concurrently with **zero** contaminating bytes on stdout and diagnostics byte-identical to the control arm — the first observation of R018 hypothesis 6. The decisive fact came after that spike: the pinned VSIX's own launcher already branches on `boot-java.ai.mcp-server-enabled` (default `false`) and `boot-java.ai.mcp-server-port` (default `50627`), pushing `-Dserver.port=<port>` when enabled and our exact flag when not. So this extension currently implements one of upstream's two branches rather than deviating from it, and the "random port announced only through `window/showMessage`" concern is an artefact of how the spike was run: upstream supplies a fixed port, and the port also reaches stderr. `not-pursued` is therefore the wrong label — it means a documented exception outside the parity target, which is true of Spring Initializr and false here. The residual costs are real and opt-in only: the endpoint is unauthenticated (loopback-bound, but any local process could enumerate beans, mappings and classpath through it), and the five spring.io tools make live outbound calls that would reopen the closed offline row for a user who enables them. Defaulting off keeps the default launch byte-identical to today's. Version 46 predicted the implementation would need a settings path this extension lacks; building it refuted that, because `boot-java.live-information.automatic-connection.on` was already read at launch and passed as `--automatic-live-connection`, and the MCP settings reuse that route. Tool **payloads** against a resolved project remain unobserved — S019 ran without the official-Java bridge, so index-backed tools were reachable but empty — and that is what a later `verified` promotion must show. |
...
| Embedded language syntax highlighting | `verified` | Verified 2026-07-29 on macOS 26.5.x arm64, **Zed 1.12.1**, official Java 6.8.21 (jdtls 1.60.0), Spring Tools 5.2.0, Temurin JDK 25.0.3, against the Boot 3.5.5 fixture. **This row was `blocked-zed-api` for eight days on a mistake of ours, and the mistake is the useful part.** [S017](spikes/017-static-semantic-token-declaration.md) concluded that Zed issues no `textDocument/semanticTokens/*` request for a Java buffer under either dynamic or static registration, and rested it on a control that looked unanswerable: jdtls, the primary Java server, declares `semanticTokensProvider` statically and drew no request either. The real cause was a client setting nobody read. Zed's `semantic_tokens` is a **language setting whose default is `"off"`**, documented in `assets/settings/default.json` as "do not request semantic tokens from language servers" — so the control proved only that the global switch was off for both servers at once. It is the same default-off trap as `document_symbols`, eight lines away in the same settings block, which this project had already been caught by once. Nothing changed upstream to unblock it: `zed#7450` (Support LSP Semantic Tokens) closed 2026-02-06 and `zed#60015` (dynamic registration) merged 2026-06-28, both before S017 ran on 1.11.3. **Two levers, and only one of them is ours.** The extension now sends `boot-java.embedded-syntax-highlighting: true` (`spring_default_configuration`): `BootJavaConfig.isJavaEmbeddedLanguagesSyntaxHighlighting()` is `isEnabled != null && isEnabled.booleanValue()`, so an absent key reads false while the VSIX schema defaults it `true` — the same parity trap as `jpql` and `inject-bean`, confirmed from the installed `vmware.vscode-spring-boot-2.2.0/package.json`. With it off, `EmbeddedLanguagesSemanticTokensSupport` strips `java` from the token document selector, so Spring never offers a Java provider at all. The other lever is the user's: **Zed's `semantic_tokens` must be set to `"combined"` or `"full"`**, and no extension can set it — that is a documented requirement, not a workaround, and it is the reason this row is `verified` in the same shape as Document Symbols rather than silently on. **No coordinator code was needed.** Spring's `client/registerCapability` passes through unchanged and Zed honours it; the static-declaration adaptation on branch `spike/s017-static-semantic-tokens` is not required and stays unmerged. *Fan-out*: one buffer gesture produced **two** outgoing `textDocument/semanticTokens/full` at the same timestamp, one per server, both answered — Zed's `fetch_semantic_tokens_for_buffer` iterates `local_lsp_servers_for_buffer` and returns a `HashMap<LanguageServerId, SemanticTokensResponse>`. This is the opposite of the single-primary `documentHighlight` path and settles the neighbouring row's "same class of missing surface" reading as wrong. *Attribution*, since Zed's `[lsp]` lines never name the server: the legends differ in size — Spring's registration legend carries **21** token types (`string`/`number`/`operator`/`macro` at 17–20), jdtls's static one **17** (max index 16) — so any response containing a token type index ≥ 17 is Spring's. Decoded that way on `GreetingRepository.java`'s JPQL text block, Spring returned 106 tokens including `select`/`from`/`where` as `keyword`, `Greeting` as `class`, `message` as `property` and `?1` as `parameter` **inside the string**, while jdtls returned 55 with nothing inside it but `decorator` on `@Query` itself; `DataQuerySample.java` reproduced it for the single-line and `nativeQuery` forms. *Render*: the screenshot shows the text block coloured accordingly with ordinary Java highlighting intact around it and the IH-2 `?1` → `message` inlay hint still drawn — S017's two success criteria, both met. **One real limitation, recorded rather than smoothed over.** On a cold start the first requests can be lost: both 4–5 s after launch returned `null` because Spring's `JdtSemanticTokensHandler.computeTokens` hit the known transient upstream `NullPointerException` (a closed `ZipFile`, the same family as the truncated first reconcile in the Spring Data query row), and **Zed caches that empty result per buffer and does not re-request on refocus** — reopening at an explicit caret produced zero new requests. Opening a different Java buffer, or any edit that reloads the buffer, gets tokens in 70–230 ms. So the first Java file of a session may render without embedded highlighting until it is touched. The opt-in Java query pack remains an independent tree-sitter route needing its own direction decision, and is now unnecessary for this outcome. Evidence: `tmp/semantic-tokens-recheck-20260729/evidence/` (superseding `tmp/s017-static-semantic-tokens-20260721/evidence/`). |
