...
| Code actions / quick fixes | `verified` | Observed 2026-07-18 on macOS arm64/JDK 25 with the development extension and Zed 1.11.3. With the cursor on the intentional `ser` unknown-property diagnostic in `application.properties`, Zed issued `textDocument/codeAction` through the coordinator and the Spring server returned a non-empty quickfix: `Create metadata for 'ser'` (kind `quickfix`, command `sts.vscode-spring-boot.codeAction`, `MISSING_PROPERTY_APP`), `source: vscode-spring-boot`. Properties routes only to `zed-spring-tools`, so it is Spring-attributed, and the offer is reachable through Zed's standard code-action surface. Executing the command (`workspace/executeCommand` → `workspace/applyEdit`) was not driven; only the offer round-trip. Evidence: `tmp/cav-verify-20260718/`. |
