...
An earlier draft of this entry claimed the slice needs a layer this extension
does not have, on the grounds that every `boot-java.*` key handled so far is
forwarded *to* the server through `spring_workspace_configuration`. That is
wrong, and the correction shrinks the slice rather than growing it:
`boot-java.live-information.automatic-connection.on` is already read from
`LspSettings` at launch time and passed to the coordinator as
`--automatic-live-connection`, which then shapes runtime behaviour. The MCP
settings follow that established route exactly — read at launch, passed as
coordinator arguments, applied in `springArguments`.
