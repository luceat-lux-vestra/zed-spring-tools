...
It measured how rarely the feature fires at all: on a plain `server.` prefix it
does nothing, because fuzzy matches like `spring.rsocket.server.port` collapse
the shared prefix. And it found the thing no amount of reading upstream would
have produced — with the setting on, Zed shows **no completion menu at all**
where the same caret with it off shows five items and accepts correctly. The
setting is not cosmetic in Zed; it removes working completions. That is now the
