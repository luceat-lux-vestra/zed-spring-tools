...
```bash
awk 'NR>=888' docs/capability-inventory.md | awk -F'|' 'NF>=4 {gsub(/[ `]/,"",$3); if ($3 ~ /^(verified|implemented|planned|blocked-zed-api|blocked-upstream|zed-native-equivalent|not-pursued)$/) print $3}' | sort | uniq -c
```
