...
| Metrics — inspect memory and request/runtime measurements | The bounded Live-document route is `implemented` but not yet driven; users may still use the application's own Actuator UI. | Route implemented: a Java source Code Action selects an already-connected process, explicitly refreshes memory/GC families, and writes finite, timestamped heap/non-heap/GC measurements to the owned `.zed/spring-live.md` without process keys or metric tags. | Contract tests closed the interaction, freshness, ownership, output-bound, and persisted-identifier gates. Keep the row `implemented` until a real connected-Boot run proves authentic values and regeneration; Actuator remains the runtime fallback. | 4/5; runtime gate pending |
...
5. The first live-data connection slice is `verified`, and the aggregate metrics
   slice is now `implemented`: its bounded process choice, explicit refresh,
   timestamped owned document, finite-value limits, and persisted-identifier
   exclusions are contract-tested. Next run that document against the connected
   Boot/JMX fixture before promoting it; loggers, automatic/remote connection,
   and remaining commands retain their separate interaction/security gates.
