...
| Live hover and inline data — show runtime bean or endpoint information at source | Source-local bean, injection and endpoint facts are verified as live CodeLens plus native Hover; aggregate metrics are separately verified, and logger aggregation/level changes are implemented pending a runtime gate. | Keep versioned live results in standard CodeLens and use Zed's composed Hover for detail until a one-click client-command bridge exists. | Reject old document versions, explain unavailable VS Code-only commands, and retain static Spring hover/Project Symbols when disconnected. | 4.5/5 |
...
| Loggers and log levels — list loggers and change a running level | `implemented`, not runtime-verified; users may still use Actuator directly. | Route implemented: append a bounded read-only logger snapshot to the opt-in Live document, and expose a separate Java source Code Action with paged logger selection, a server-advertised level, final confirmation, and success keyed to the matching `sts/liveprocess/loglevel/updated` notification. | The document stays read-only because stock Zed cannot safely attach commands to generated Markdown. Omitted entries and an unavailable endpoint fall back to Actuator; dismissal, disconnect, timeout, and mismatched updates fail closed. | 4/5; runtime gate pending |
...
5. The first live-data connection and aggregate metrics slices are `verified`,
   and the logger slice is `implemented` pending its connected-Boot runtime gate.
   ...
   connected Boot/JMX fixture. Logger inspection/configuration is contract-tested
   with bounded document and confirmation rules, but still needs authentic
   `getLoggers`, `configure/logLevel`, update-notification and Zed-render gates.
   Automatic/remote connection and remaining commands retain their separate
   interaction/security gates.
