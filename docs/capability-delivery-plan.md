...
| Embedded syntax highlighting — highlight SpEL, JPQL, and query fragments inside Java strings | Preserve official Java highlighting without Spring-specific embedded grammar. | **Route taken and `verified` 2026-07-29, reversing S017.** The 2026-07-21 spike read Zed's silence as a missing request/render path; the cause was Zed's `semantic_tokens` language setting defaulting to `"off"`, which suppressed requests to both servers and so made jdtls's static declaration a control that shared its cause with the treatment. The extension now sends `boot-java.embedded-syntax-highlighting: true`, the same absent-key-reads-false trap as `jpql`; the coordinator needs no change, because Spring's dynamic registration passes through and Zed honours it. | The user must set Zed's `semantic_tokens` to `"combined"` or `"full"`; no extension can set it, so it is documented, and tree-sitter Java highlighting is the unchanged fallback when it is off. Spring answers for the whole Java file, so `embedded-syntax-highlighting: false` is the way back without losing the official Java server's tokens. Never risk the whole Java language registration for this enhancement. | 5/5 on the tested tuple, under the documented client setting |
...
3. Take WS2's remaining language-intelligence rows. Spring XML config is now
   ...
   so the Spring-specific highlight slice is `blocked-zed-api`. (**Corrected
   2026-07-29:** this read "the same class as the S017 semantic-token result".
   It is not — S017 was a default-off client setting and its row is now
   `verified`. Semantic tokens fan out to every capable server; document
   highlights are the request that stops at `LanguageServerToQuery::FirstCapable`
   in Zed's source. The highlight evidence itself is unaffected.) Do not build
   coordinator highlight code.
   **SpEL is now `verified` too, and closed with no product code.** A source
   ...
   tokens were taken as refuted below (overturned 2026-07-29; SpEL tokens now
   render under the same setting), and the pinned release has no SpEL hover
   provider
...
4. The semantic-token spike was **closed as refuted** on 2026-07-21 and is
   **overturned as of 2026-07-29**
   ([S017](spikes/017-static-semantic-token-declaration.md) carries both). Zed
   requested nothing after either dynamic or static registration — including
   after jdtls's own static declaration, which is what made the conclusion feel
   decisive — because Zed's `semantic_tokens` language setting defaults to
   `"off"` and suppresses requests to every server at once. Re-driven on Zed
   1.12.1 with that setting `"combined"`, Zed fanned the request to both
   servers, Spring returned tokens inside the `@Query` text, and the editor
   rendered them, so *Embedded language syntax highlighting* is `verified` and
   token-level colouring is back in scope alongside the diagnostics, hover and
   navigation that never depended on it.
