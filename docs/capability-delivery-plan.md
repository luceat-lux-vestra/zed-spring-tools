...
| Local process connection — connect live data to a running Boot process | Now `implemented` (contract-tested; driven gate pending). No reduced connection mode is claimed. | Implemented: a Code Action runs `sts/livedata/listProcesses`, presents a bounded Zed message choice, executes the descriptor's own connect/disconnect/refresh action, and keys connect success off the `sts/liveprocess/connected` notification with coordinator-owned connection state and cleanup. | If the process list exceeds a usable prompt or identity is ambiguous, use the opt-in Live document; a failed connect is reported as a bounded request, never a false success. | 3.5/5; driven gate pending |
