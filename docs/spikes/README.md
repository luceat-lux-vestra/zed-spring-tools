...
| [S018](018-references-highlights-multiserver-composition.md) | Observe how stock Zed composes two servers advertising `referencesProvider`/`documentHighlightProvider` for one Java buffer (synthetic; no product code) | Mixed on macOS arm64/JDK 25/Zed 1.11.3: `textDocument/references` fans to both servers and **unions** the results (Spring's `@Qualifier`→`@Bean` link surfaces beside jdtls's Java refs) → Supported; `textDocument/documentHighlight` goes to the **single primary** (jdtls only), so Spring highlights cannot surface → Refuted for the Spring slice |
