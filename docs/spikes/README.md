...
latest-release lookup. S008 now has a reviewed plan to preseed both fixed inputs
in a new minimal profile and repeat the managed data-path observation twice;
Gate A has not started.
...
| [S008](008-preseeded-managed-jdt-isolation.md) | Repeat managed-local JDT isolation twice with fixed helper/catalog inputs | Plan reviewed; Gate A not started |
