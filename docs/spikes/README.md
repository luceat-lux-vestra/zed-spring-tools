...
| [S017](017-static-semantic-token-declaration.md) | Test whether Zed requests semantic tokens when the coordinator declares `semanticTokensProvider` statically in the `initialize` result, after the dynamic-registration path produced no request | **Refuted 2026-07-21, overturned 2026-07-29.** The 1.11.3 run saw no semantic-token request for a Java buffer under either registration shape — including from jdtls, the primary server — and the row was promoted to `blocked-zed-api`. The re-run on 1.12.1 found the cause was Zed's `semantic_tokens` language setting, which defaults to `"off"` and therefore suppressed requests to both servers at once, making the jdtls "control" share a cause with the treatment. With it set to `"combined"` the embedded JPQL/HQL highlighting renders and the row is `verified`; the reference impl on `spike/s017-static-semantic-tokens` is not needed and stays unmerged |
| [S018](018-references-highlights-multiserver-composition.md) | Observe how stock Zed composes two servers advertising `referencesProvider`/`documentHighlightProvider` for one Java buffer (synthetic; no product code) | Mixed on macOS arm64/JDK 25/Zed 1.11.3: U3 showed `textDocument/references` fans to both servers and **unions** the results; U4 then verified Spring's `@Qualifier`→`@Bean`, `@Value`→`application.properties`, and distinct `@Named` injection→bean targets in the composed References result → Supported. `textDocument/documentHighlight` goes to the **single primary** (jdtls only), so Spring highlights cannot surface → Refuted for the Spring slice |
| [S019](019-embedded-mcp-server.md) | Test whether the pinned jar's embedded MCP server starts once `-Dspring.main.web-application-type=NONE` is lifted, whether one JVM serves streamable-HTTP MCP and stdio LSP at once, and how the random port is announced (decision input; no product code) | Mixed on macOS arm64/JDK 25: the flag alone gates it — dropping it bound `127.0.0.1:<random>`, `POST /mcp` returned all 18 tools, and the LSP stream stayed byte-clean with diagnostics identical to the control, closing R018 hypothesis 6 → Supported. The port is **not** announced only via `window/showMessage`; Tomcat also logs it to stderr → Refuted for that clause |
