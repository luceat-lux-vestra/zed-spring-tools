...
| [S017](017-static-semantic-token-declaration.md) | Test whether Zed requests semantic tokens when the coordinator declares `semanticTokensProvider` statically in the `initialize` result, after the dynamic-registration path produced no request | Refuted on macOS arm64/JDK 25/Zed 1.11.3: Zed issues no semantic-token request for a Java buffer even when the provider is declared statically — including by jdtls, the primary server. Row promoted to `blocked-zed-api`; reference impl on `spike/s017-static-semantic-tokens`, not in the product |
