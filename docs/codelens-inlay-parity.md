...
| CL-4 | Data AOT metadata lenses (a–e) | Show/generate AOT impl, generated query text, `Turn into @Query`, Go To Implementation, Refresh | `DataRepositoryAotMetadataCodeLensProvider` | Preserve the refactoring command; pre-resolve authentic `CL-4d` target and rewrite to Zed navigation; answer the build command with a reviewable `.zed/tasks.json` entry rather than Spring's in-process build | `verified`, including the driven build-task route |
